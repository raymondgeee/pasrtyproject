...
$batchNumberDatas = isset($_REQUEST['batchNumber']) ? $_REQUEST['batchNumber'] : "";
...
if ($batchNumberDatas != "") 
{
    $includeQuery .= " AND batchNumber = '".$batchNumberDatas."'";
}
...
{
    $includeQuery .= " AND orderStatus NOT IN (4)";
    // if($chkDenied == "")
    // {
...
$userIdArray = $orderStatusArray = Array ();
...
$productOrderPriceArray = $batchNumberDataArray = Array ();
...
if($queryReviewOrders AND $queryReviewOrders->num_rows > 0)
{
    while ($resultReviewOrders = $queryReviewOrders->fetch_assoc())
    {
        ...
        $batchNumberDataArray[] = $resultReviewOrders['batchNumber'];
                                            ...
                                            <div class='row'>
                                                ...
                                                <div class='col-md-2'>
                                                    ...
                                                    <select form="formFilter" class="w3-select w3-border w3-small" id='batchNumber' name='batchNumber' type="combobox">
                                                        <option value="">Choose your option</option>
                                                        <?php
                                                        $batch = array_unique($batchNumberDataArray);
                                                        foreach ($batch as $key) 
                                                        {
                                                            $selectedBatch = ($batchNumberDatas == $key) ? "selected" : '';
